  ...
  if (!optional) {
    if (data instanceof Array || data instanceof Uint8Array) {
      for (i = 0; i < data.length; i++) {
        datum = data[i];
        if (datum !== null) {
          // nulls and undefineds are silently skipped.
      ...
      for (i = 0; i < data.length; i++) {
        ...
        if (datum < 0 || datum > 255) {
          throw new Error(
            "Characters above 255 not allowed without explicit encoding: " +
              datum
          );
        }
        this.array.push(datum);
...
png.Bytes.prototype.toString = function (n) {
  ...
  for (var i = 0; i < chunks.length; i++) {
    ...
    for (var j = 0; j < chunks[i].length; j++) {
      ...
      chr.push(byte >= 32 && byte <= 126 ? String.fromCharCode(byte) : "_");
    }
...
png.fromRaster = function (raster, optional_palette, optional_transparency) {
  ...
  var plte =
    optional_palette instanceof Array
      ? new png.Chunk.PLTE(optional_palette)
      : new png.Bytes([]);
  var trns =
    optional_transparency instanceof Array
...
png.Chunk = function (type, data) {
  ...
  if (!type.match(/^[A-Za-z]{4}$/)) {
    throw new Error(
      "Creating PNG chunk: provided type should be four letters, not " + type
    );
  }

  if (!(data instanceof png.Bytes)) {
    throw new Error(
      "Creating PNG " + type + " chunk: provided data is not Bytes: " + data
    );
  }
  ...
  var length_type_data_checksum = new png.Bytes(data.array.length, { bytes: 4 })
    .add(type_and_data)
    .add(crc, { bytes: 4 });
...
png.Chunk.IHDR = function (width, height, bit_depth, color_type) {
  ...
  return new png.Chunk(
    "IHDR",
    new png.Bytes(width, { bytes: 4 }).add(height, { bytes: 4 }).add([
      bit_depth,
      color_type,
...
png.Chunk.tRNS = function (alpha_list) {
  ...
  return new png.Chunk("tRNS", new png.Bytes(alpha_list));
};
...
png.Raster = function (bit_depth, color_type, raster) {
  ...
  for (var row = 0; row < raster.length; row++) {
    if (raster[row].length != cols)
      throw new Error(
        "Row " + row + " does not have the expected " + cols + " columns."
      );
  ...
  this.encode = function () {
    ...
    for (var row = 0; row < raster.length; row++) {
      ...
      for (var col = 0; col < cols; col += 8 / bit_depth) {
        ...
        if (byte & ~0xff)
          throw new Error(
            "Encoded raster byte out of bounds at (" + row + "," + col + ")"
          );
        buffer.push(byte);
      }
    }
...
png.Raster_rgb = function (bit_depth, color_type, raster) {
  ...
  for (var row = 0; row < raster.length; row++) {
    ...
    for (var col = 0; col < cols; col++) {
      if (
        !(color_type == 2 && raster[row][col].length == 3) &&
