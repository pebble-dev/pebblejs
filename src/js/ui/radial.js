...
var accessorProps = ["radius", "angle", "angle2"];
...
var checkProps = function (def) {
  ...
  if ("angleStart" in def && safe.warnAngleStart !== false) {
    safe.warn(
      "`angleStart` has been deprecated in favor of `angle` in order to match\n\t" +
  ...
  if ("angleEnd" in def && safe.warnAngleEnd !== false) {
    safe.warn(
      "`angleEnd` has been deprecated in favor of `angle2` in order to match\n\t" +
