...
var WindowTypes = ["window", "menu", "card"];

var WindowType = makeArrayType(WindowTypes);

var ButtonTypes = ["back", "up", "select", "down"];

var ButtonType = makeArrayType(ButtonTypes);

var ButtonFlagsType = makeFlagsType(ButtonTypes);

var CardTextTypes = ["title", "subtitle", "body"];

var CardTextType = makeArrayType(CardTextTypes);

var CardTextColorTypes = ["titleColor", "subtitleColor", "bodyColor"];

var CardImageTypes = ["icon", "subicon", "banner"];
...
var VibeTypes = ["short", "long", "double"];

var VibeType = makeArrayType(VibeTypes);

var LightTypes = ["on", "auto", "trigger"];
...
var StatusBarSeparatorModeTypes = ["none", "dotted"];
...
var ReadyPacket = new struct([[Packet, "packet"]]);
...
var AccelPeekPacket = new struct([[Packet, "packet"]]);
...
var MenuClearPacket = new struct([[Packet, "packet"]]);
...
var MenuGetSelectionPacket = new struct([[Packet, "packet"]]);
...
var StageClearPacket = new struct([[Packet, "packet"]]);
...
var VoiceDictationStopPacket = new struct([[Packet, "packet"]]);
...
PacketQueue.prototype._maxPayloadSize =
  (Platform.version() === "aplite" ? 1024 : 2044) - 32;
...
SimplyPebble.wakeupSet = function (timestamp, cookie, notifyIfMissed) {
  WakeupSetPacket.timestamp(timestamp)
    .cookie(cookie)
...
SimplyPebble.windowProps = function (def) {
  WindowPropsPacket.prop(def).backgroundColor(def.backgroundColor || "white");
...
SimplyPebble.windowStatusBar = function (def) {
  ...
  WindowStatusBarPacket.separator(statusDef.separator || "dotted")
...
SimplyPebble.windowActionBar = function (def) {
  ...
  WindowActionBarPacket.up(actionDef.up)
...
SimplyPebble.menuSection = function (section, def, clear) {
  ...
  MenuSectionPacket.section(section)
    .items(def.items)
...
SimplyPebble.menuItem = function (section, item, def) {
  MenuItemPacket.section(section)
    .item(item)
    .icon(def.icon)
    .titleLength(def.title)
...
SimplyPebble.menuSelection = function (section, item, align) {
  ...
  SimplyPebble.sendPacket(
    MenuSelectionPacket.section(section)
      .item(item)
      .align(align || "center")
  );
};
...
SimplyPebble.elementCommon = function (id, def) {
  ...
  ElementCommonPacket.id(id).prop(def);
...
SimplyPebble.elementAnimate = function (id, def, animateDef, duration, easing) {
  ...
  ElementAnimatePacket.id(id).duration(duration).easing(easing);
...
SimplyPebble.onLaunchReason = function (packet) {
  ...
  if (isTimezone) {
    ...
    state.timeOffset =
      Math.round((remoteTime - time) / resolution) * resolution;
  }
